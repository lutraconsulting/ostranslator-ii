...
class Styler(object):
    ...
    def populate_locations(self):
        # updated to reflect the actual repository structure
        # base_url = 'https://raw.githubusercontent.com/OrdnanceSurvey/OSMM-Topography-Layer-stylesheets/main'
        base_url = 'https://raw.githubusercontent.com/OrdnanceSurvey/OSMM-Topography-Layer-stylesheets/master'
        if self.osmm_schema == '7':
            # updated to reflect the actual repository structure
            # qml_base = base_url + '/stylesheets/qgis/schema7/'
            # sql_base = base_url + '/tools/PostgreSQL/schema7/'
            qml_base = base_url + '/Schema%20version%207/Stylesheets/QGIS%20stylesheets%20(QML)/'
            sql_base = base_url + '/Schema%20version%207/SQL/PostGIS/Array/'
            
            sql_mode = '_createtable_array.sql'

            # updated QML filenames to match the new repository structure
            qml_map = {
                'topographicarea': 'OSMM Topo - Topographic Area.qml',
                'cartographicsymbol': 'OSMM Topo - Cartographic Symbol.qml',
                'cartographictext': 'OSMM Topo - Cartographic Text.qml',
                'boundaryline': 'OSMM Topo - Boundary Line.qml',
                'topographicline': 'OSMM Topo - Topographic Line.qml',
                'topographicpoint': 'OSMM Topo - Topographic Point.qml'
            }

            for t in self.styleSupportedTopoTables:
                self.qmlLocations[t] = f'{qml_base}{quote(qml_map[t])}'
                self.sqlLocations[t] = f'{sql_base}{t}{sql_mode}'

        elif self.osmm_schema == '9':
            allowed_style_names = ["backdrop", "light", "standard", "outdoor"]
            if self.osmm_style_name not in allowed_style_names:
                self.osmm_style_name = "standard"
            # updated to reflect the actual repository structure
            # qml_base = base_url + '/stylesheets/qgis/'
            # sql_base = base_url + '/tools/PostgreSQL/'
            qml_base = base_url + '/Schema%20version%209/Stylesheets/QGIS%20stylesheets%20(QML)/'
            sql_base = base_url + '/Schema%20version%209/SQL/PostGIS/Array/'
            ...
            for t in self.styleSupportedTopoTables:
                if t == "boundaryline" and self.osmm_style_name == "outdoor":
                    ...
                    qml_mode = f'-{self.osmm_style_name}.qml'

                self.qmlLocations[t] = f'{qml_base}{t}{qml_mode}'
                self.sqlLocations[t] = f'{sql_base}{t}{sql_mode}'

        else:
            raise Exception(f"Unsupported OSMM schema {self.osmm_schema}")
