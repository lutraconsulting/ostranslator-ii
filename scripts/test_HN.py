...
def check_tables():
    connection = pg.connect('dbname=ostII user=postgres password=postgres host=localhost port=5432')
    schema = 'test'
    # for Roads
    ...
    tables = ['structure', 'turnrestriction']
    for t in tables:
        columns = get_table_columns(connection, schema, t)
        for c in columns:
            try:
                cur = connection.cursor()
                sql = "SELECT DISTINCT {} FROM {}.{} LIMIT 2".format(c, schema, t)
                cur.execute(sql)
